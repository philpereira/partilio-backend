...
export class CSVController {
  ...
  static async importExpenses(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (!req.file) {
        res.status(400).json({
          success: false,
          message: 'Arquivo CSV nÃ£o fornecido',
        } as ApiResponse);
        return;
      }
      ...
      if (!validation.success) {
        res.status(400).json({
          success: false,
          message: 'Mapeamento de campos invÃ¡lido',
      ...
      res.status(statusCode).json({
        ...
        message: result.success 
          ? `ImportaÃ§Ã£o concluÃ­da! ${result.importedCount} despesas importadas${result.errorCount > 0 ? ` com ${result.errorCount} erros` : ''}.`
          : 'Falha na importaÃ§Ã£o. Verifique os erros e tente novamente.',
        data: {
          importedCount: result.importedCount,
          errorCount: result.errorCount,
  ...
  static async exportExpenses(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json({
        success: false,
        message: 'Erro interno durante a exportaÃ§Ã£o',
      } as ApiResponse);
    }
  }
  ...
  static async getStats(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json({
        success: false,
        message: 'Erro interno ao obter estatÃ­sticas',
      } as ApiResponse);
    }
  }
}
